...
class StockDataHandler:
    def __init__(self, ticker):
        self.ticker = ticker
        self.data_path = os.path.join("data", f"{ticker}.csv")
...
def create_gradio_interface():
    ...
    def resolve_prediction_days(prediction_duration):
      if isinstance(prediction_duration, int):
          return prediction_duration  # already days

      duration_map = {
          "5m": 0,   # intraday, not supported in day-based prediction
          "15m": 0,
          "30m": 0,
          "1h": 0,
          "4h": 0,
      ...
      return duration_map.get(prediction_duration, 1)  # default to 1day
    ...
    def user_predict_stock(ticker, duration):
        ...
        duration = resolve_prediction_days(duration)
