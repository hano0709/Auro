#%%writefile stock_prediction_transformer_intraday.py
...
class StockDataHandler:
    def __init__(self, ticker):
        self.ticker = ticker
        self.data_path = os.path.join("data", f"{ticker}_intraday.csv")
        self.api_key = ALPHA_API_KEY
        # Ensure 'data' directory exists
        os.makedirs("data", exist_ok=True)
        self.scaler = MinMaxScaler(feature_range=(0, 1))

    def fetch_data(self, interval=1):
        """Fetch intraday stock data from Alpha Vantage and save to CSV"""
        try:
            print(f"Fetching intraday data for {self.ticker} using Alpha Vantage...")

            url = (
                f'https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY'
                f'&symbol={self.ticker}&interval=1min&outputsize=full&apikey={ALPHA_API_KEY}'
            )
            ...
            # Try to automatically find the time series key
            ...
            print(f"Downloaded {len(df)} data points for {self.ticker}")
            print(f"Data saved to {self.data_path}")
            return df

        except Exception as e:
            print(f"Error fetching intraday data for {self.ticker}: {str(e)}")
            import traceback
...
class ModelTrainer:
    ...
    def save_model(self, path):
        """Save model to disk"""
        print("Current Working Directory:", os.getcwd())  # Check where it's running
...
class StockManager:
    ...
    def predict_stock(self, ticker, duration):
    ...
    def get_stock_summary(self, ticker):
        ...
        try:
            ...
            try:
                week_change = ((current_price - float(data['Close'].iloc[-1950])) / float(data['Close'].iloc[-1950])) * 100
            except:
                week_change = None

            try:
                month_change = ((current_price - float(data['Close'].iloc[-8970])) / float(data['Close'].iloc[-8970])) * 100
            except:
                month_change = None
            ...
            ax.plot(data.index[-500:], data['Close'].iloc[-500:])
            ax.set_title(f"{ticker} - Last 500 Minutes")
            ax.set_xlabel("Time")
            ax.set_ylabel("Price")
            ax.grid(True)
